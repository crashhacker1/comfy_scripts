...
WEBSITE="$1"
echo "Getting jpeg list..."
lynx -cache=0 -dump -listonly "$WEBSITE" | grep ".*\.jpeg$" | awk '{print $2}' | tee jpeg_pic_links.txt
echo "Downloading jpeg_pics..."    
wget -nc -P ./jpeg_pics -i jpeg_pic_links.txt
...
lynx -cache=0 -dump -listonly "$WEBSITE" | grep ".*\.pdf$" | awk '{print $2}' | tee pdf_pic_links.txt
echo "Downloading pdf_pics..."    
wget -nc -P ./pdf_pics -i pdf_pic_links.txt
